...
PrimeFaces.widget.Poseidon = PrimeFaces.widget.BaseWidget.extend({
    ...
    saveMenuState: function() {
        $.cookie('poseidon_expandeditems', this.expandedMenuitems.join(','), {path: getContextPath()});
    },
    
    clearMenuState: function() {
        $.removeCookie('poseidon_expandeditems', {path: getContextPath()});
    },
    
    setInlineProfileState: function(expanded) {
        if(expanded)
            $.cookie('poseidon_inlineprofile_expanded', "1", {path: getContextPath()});
        else
            $.removeCookie('poseidon_inlineprofile_expanded', {path: getContextPath()});
    },
