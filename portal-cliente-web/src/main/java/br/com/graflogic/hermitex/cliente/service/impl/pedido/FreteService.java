			...
			for (String servicoFrete : servicosFrete) {
				...
				tipoProdutoFor: {
					for (String tipoProduto : tiposProduto) {
						List<PedidoFrete> pedidoFretes = new ArrayList<>();
							...
							if (null != embalagem) {
								...
								for (CServico servico : resultado.getServicos().getCServico()) {
									...
									if (valor.compareTo(BigDecimal.ZERO) > 0) {
										...
										pedidoFrete.setIdEmbalagem(embalagem.getId());
										pedidoFrete.setPesoItens(pesoFrete);
										pedidoFrete.setQuantidadeItens(quantidadeFrete);
										pedidoFrete.setNomeEmbalagem(embalagem.getNome());
										pedidoFrete.setValor(valor);
										pedidoFrete.setCodigoServico(servicoFrete);
										pedidoFrete.setPrazoDias(Integer.parseInt(servico.getPrazoEntrega()));

										pedidoFretes.add(pedidoFrete);
						...
						fretesTipoFrete.addAll(pedidoFretes);
					}
				}
